...
// Für CUDA 12.x Kompatibilität
#if __CUDA_ARCH__ >= 700
#define USE_MODERN_CUDA 1
#else
#define USE_MODERN_CUDA 0
#endif

#include <cuda_runtime.h>
#include <device_launch_parameters.h>

// Für bessere Kompatibilität mit neueren CUDA-Versionen
...
__device__ __noinline__ void CheckPoint(uint32_t* _h, int32_t incr, int32_t mode,
...
__device__ __noinline__ void CheckPoint2(uint32_t* _h, int32_t incr, int32_t mode,
...
#define CHECK_POINT(_h,incr,mode)  CheckPoint(_h,incr,mode,bloomLookUp,BLOOM_BITS,BLOOM_HASHES,maxFound,out,P2PKH)
	...
	uint8_t* bloomLookUp, int BLOOM_BITS, uint8_t BLOOM_HASHES, uint32_t maxFound, uint32_t* out)
{
	...
	CHECK_POINT(h, incr, true);

}


#define CHECK_POINT2(_h,incr,mode)  CheckPoint2(_h,incr,mode,hash160,maxFound,out,P2PKH)
...
CheckPoint(h1, (_incr), true, bloomLookUp, BLOOM_BITS, BLOOM_HASHES, maxFound, out, P2PKH);     \
}
...
_GetHash160CompSym(px, (uint8_t *)h1, (uint8_t *)h2);              \
CheckPoint2(h1, (_incr), true, hash160, maxFound, out, P2PKH);     \
}
